...
(defun http-parse (parser callbacks data &key (start 0) end)
  ...
  (let ((mark (init-mark (parser-mark parser)))
    ...
    (declare (type pointer end)
             (type fixnum header-read))

    (labels ((finalize ()
               (setf (parser-header-read parser) header-read))
             (return-with-finalize (i)
               (finalize)
               (return-from http-parse i)))
      (unless (= (parser-http-errno parser) +ok+)
        (return-with-finalize 0))
